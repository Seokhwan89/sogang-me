...
- **BK21 홈페이지 링크 복원 (박정열 교수님 요청 2026-09-02)**: 옛 사이트 상단바에 있던 BK21 링크(http://bk21me.sogang.ac.kr — 사이트 자체는 계속 살아 있음, 옛 서버와 무관한 별도 호스팅)가 새 사이트에 없었음 → 대학원과정 메뉴·페이지 탭·푸터에 외부 링크로 추가(새 탭, 사이트맵 제외). nav에 http로 시작하는 항목은 외부 링크로 렌더링됨(lib/nav.ts isExternal)
