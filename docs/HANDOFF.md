...
## 진행 중 — 다음 세션이 이어서 할 것
- **디자인 개편 브랜치 미리보기 확인 대기**: `claude/handoff-legacy-content-cleanup-av86zr`에 홈 히어로 12영상 순환(분야 교차, CSS 폴백)·콘셉트 이미지 전면 교체·페이지 히어로 SVG 애니메이션 장식·글라스 헤더·산학 트랙 기업 로고 배너·홈 캐러셀 20건+전체보기 카드가 올라가 있음(빌드 통과). **책임자 미리보기 승인 후 main 병합**하면 됨. 미디어는 Mixkit 무료 라이선스에서 추출해 public/media 자체 호스팅(출처는 assets.ts 주석), 기업 로고는 공식 산학 트랙 안내 목적 표기
- 옛 사이트 공용서버 예약 달력 2026-10월분만 프록시 문제로 미확인(8·9월은 빈 달력) — 도메인 만료 전 한 번 재확인 권장
