...
## 보류 · 대기
- **[해야 함] `supabase/schema_v8.sql` 적용**: Supabase 대시보드(supabase.com, 학과 Gmail 로그인) → 프로젝트 → SQL Editor → 파일 내용 붙여넣고 Run. 내용: ① 예약자 연락처 익명 노출 차단(컬럼 grant) ② 관리자 추가 쓰기 정책 ③ URECA 재제출 대체 RPC ④ 예약 insert 조건 DB 강제 ⑤ 예약 레이스 회수 RPC. **적용 전에도 사이트는 정상 동작**하지만, ①의 개인정보 노출은 SQL을 적용해야 실제로 막힌다
