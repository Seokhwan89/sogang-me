  ...
  return (
    <Link href={`/${locale}/board/${post.board}/${post.id}`} className="card group flex flex-col overflow-hidden">
      <div className="aspect-[16/10] bg-sg-mist relative overflow-hidden">
        <img src={post.thumbnail_url || post.images?.[0]?.url || coverFor(post.board, post.title_ko, post.id)} alt="" loading="lazy" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-[1.04]" />
